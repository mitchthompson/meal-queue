...
| `@media (pointer: coarse) and (max-width: 1024px)` | portrait tablets | `.page-col` drops its desktop-tier `max-width: 640px` (→ `max-width: none; margin-inline: auto`) so the reading column fills the centered 960 shell instead of stranding a right gutter under the phone chrome. Landscape iPads (>1024px) and desktop keep the 640px column. |
